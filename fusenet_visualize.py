...
class Visualize:
    def __init__(self, opt, model=None, test_loader=None):
        ...
        self.new_image = Image.new('RGB', (320 * 3, 240))
    ...
    def visualize_predictions(self):
        ...
        for num, batch in enumerate(self.test_loader):
            ...
            # gray_image = np.array(test_seg_preds)
            # gray_image = gray_image * 50
            # Gray_img = Image.fromarray(np.uint8(gray_image))
            # Gray_img.save("results/predict.png")


            # Horizontally stack the predicted and ground-truth semantic segmentation labels
            comparison_images = np.hstack((np.uint8(test_seg_labels-1), np.uint8(test_seg_preds)))
