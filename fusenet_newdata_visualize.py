...
from utils.data_utils import get_data_nolabel
...
class Visualize:
    def __init__(self, opt, model=None, test_loader=None):
        ...
        if self.dset_name.lower().find('nyu') is not -1:
            ...
            self.dset_info = {'selfmade_dataset': 6}
        ...
        self.scene_class_dict = {0: '_background_', 1: 'chair', 2: 'cylinder', 3: 'foot',
                                 4: 'short-box', 5: 'tall-box'}
        ...
        self.new_image = Image.new('RGB', (640*2, 480))
    ...
    def paint_and_save(self, image, rgb_image, scene_label, scene_pred, idx):
            ...
            self.new_image.save(os.path.join(self.save_path,  str(idx) + '.png'))
            print('[PROGRESS] Saving images: %i of %i     ' % (idx+1, len(self.test_loader)))
    ...
    def visualize_predictions(self):
        ...
        if self.test_loader is None:
            _, test_data = get_data_nolabel(self.opt, use_train=False, use_test=True)
        ...
        for num, batch in enumerate(self.test_loader):
            ...
            test_rgb_name = test_data.rgb_images[num]
            test_rgb_name = test_rgb_name.replace('./datasets/out/test_rgb/','')
            test_rgb_name = eval(test_rgb_name.replace('.png',''))
            ...
            # print(test_seg_outputs[0,4,:,:])
            # print(test_seg_outputs[0,5,:,:])
            ...
            comparison_images = np.uint8(test_seg_preds)
            ...
            self.paint_and_save(comparison_images, np.uint8(test_rgb_inputs), test_class_labels, test_class_preds, test_rgb_name)
