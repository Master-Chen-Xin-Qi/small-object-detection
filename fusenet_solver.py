...
class Solver(object):
    ...
    def __init__(self, opt, dset_info, k, loss_func=torch.nn.CrossEntropyLoss):
        self.opt = opt
        self.dset_name, self.seg_class_num = next(iter(dset_info.items()))
        self.use_class = opt.use_class
        #self.vis_opt = TestOptions().parse()
        self.k = k
    ...
    def save_checkpoint(self, state, lam, is_best, k, is_current = False):
        """ Write docstring
        """
        ...
        if is_best:
            best_model_filename = os.path.join(checkpoint_dir, 'best_model' + '_{}'.format(k)+lam_text + '.pth.tar')
            ...
            return
        if is_current:
            checkpoint_filename = os.path.join(checkpoint_dir, 'current_model_checkpoint') + '.pth.tar'
        ...
        # Save checkpoint with the name including epoch, - if exists, lambda value for classification - and date
            ...
            checkpoint_filename = os.path.join(checkpoint_dir, 'model_checkpoint' +'_{}'.format(k)+ lam_text + '_{}'.format(state['epoch'] + 1)
    ...
    def update_learning_rate(self, optim, epoch):
        """
        Sets the learning rate to the initial LR decayed by 0.95 every 25 epochs.
        """
        lr = self.optim_args['lr'] * (0.95 ** (epoch // 25))
    ...
    def validate_model(self, val_loader, vis_results=True, outTrain=False):
    ...
    def train_model(self, train_loader, val_loader, num_epochs=100, log_nth=0, lam=None):
        """
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            for i, data in enumerate(train_loader):
                ...
                #print(torch.max(train_seg_labels))
                ...
                # if(epoch %20 == 10):
                #     train_seg_preds = train_seg_preds.squeeze(-3)
                #     gray = np.array(torch.Tensor.cpu(train_seg_preds))
                #     gray = gray*50
                #     img = Image.fromarray(np.uint8(gray))
                #     img.save('results/pre.png')

                labels_mask = train_seg_labels > 0
                train_seg_labels = train_seg_labels - 1
                #print(np.mean((train_seg_preds == train_seg_labels)[labels_mask].data.cpu().numpy()))
                #print(np.mean((train_seg_preds == train_seg_labels).data.cpu().numpy()))
                train_seg_scores.append(np.mean((train_seg_preds == train_seg_labels)[labels_mask].data.cpu().numpy()))
                #train_seg_scores.append(np.mean((train_seg_preds == train_seg_labels).data.cpu().numpy()))
            ...
            # self.save_checkpoint(self.update_model_state(optim), lam, False, True)
            ...
            if (epoch+1) > self.opt.save_epoch_freq:
                ...
                if is_best or (epoch+1) % 30 == 0:
                    self.states['best_val_seg_acc'] = max(current_val_seg_acc, best_val_seg_acc)

                    # model_state = self.update_model_state(epoch, self.model)
                    self.save_checkpoint(self.update_model_state(optim), lam, is_best, self.k)
        best_iou=self.states['best_val_seg_acc']
